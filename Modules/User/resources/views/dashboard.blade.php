                ...
                <div class="card-body">
                    <ul class="list-group">
                        @foreach($upcomingBillingClients as $client)
                            <li class="list-group-item">{{ $client->name }} - Billing Date: {{ $client->invoices->first()->due_date->format('d M Y') }}</li>
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($clientsInArrears as $client)
                                <tr>
                                    <td>{{ $client->client_name }}</td>
                                    <td>{{ $client->location }}</td>
                                    <td>{{ $client->email }}</td>
                                    <td>Unpaid</td>
                                    <td>{{ now()->diffInDays($client->invoices->first()->due_date) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @foreach($upcomingBillingClients as $client)
                                <tr>
                                    <td>{{ $client->name }}</td>
                                    <td>{{ $client->location }}</td>
                                    <td>{{ $client->invoices->first()->due_date->format('d M Y') }}</td>
                                    <td>{{ $client->invoices->first()->amount }}</td>
                                    <td>Pending</td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    const activityChart = new Chart(ctxActivity, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: [{{ $paidClientsCount }}, {{ $arrearsClientsCount }}, {{ $upcomingClientsCount }}],
